...
import {
  ...
  IconBrandLinkedin,
  IconBrandXing,
} from '@tabler/icons-react'; // Updated to include necessary icons
...
import desktopPaymeBannerImage from '@/public/images/Payme_Banner-970x250.jpg'; // Imported desktop ad image
import mobilePaymeBannerImage from '@/public/images/Payme_Banner_300x250.jpg'; // Imported mobile ad image

import { isMobile as checkIsMobile } from '../utils/device'; // Imported isMobile utility
...
export default function Post() {
  ...
  const renderTempPaymeAd = useCallback(
    (position: string) => {
      const mobileCheck = checkIsMobile(userAgent);
      ...
      if (position === 'right') {
        imageSrc = mobilePaymeBannerImage.src;
        imageWidth = 300;
        imageHeight = 250;
      } else {
        imageSrc = mobileCheck ? mobilePaymeBannerImage.src : desktopPaymeBannerImage.src;
        imageWidth = mobileCheck ? 300 : 970;
        imageHeight = mobileCheck ? 250 : 250;
      }
  ...
  const checkAdContent = (adId: string, setHasAd: React.Dispatch<React.SetStateAction<boolean>>) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      checkAdContent('div-gpt-ad-1728872497774-0', setHasMiddleAd); // 970x250
      checkAdContent('div-gpt-ad-1728872758865-0', setHasRightAd); // 300x250
      checkAdContent('div-gpt-ad-1728872238496-0', setHasTopAd); // 728x90 (Kept as is)
      checkAdContent('div-gpt-ad-1728872258333-0', setHasBottomAd); // 728x90 (Kept as is)
  ...
  // Commented out unused useEffect
  /*
  useEffect(() => {
    if (id) {
  ...
  */
  ...
  return (
    ...
    >
      {/* 
        Load Google Ads
        Commented out specific ad slots to replace with temporary ads.
      */}
      <Script id="gpt-init" strategy="afterInteractive">
        {`window.googletag = window.googletag || {cmd: []};
          googletag.cmd.push(function() {
            // Existing Desktop Ads
            ...
            // Commented out 970x250 and 300x250 Ads
            /*
            ...
            */
            ...
            googletag.pubads().setTargeting('site', ['playeateasy.com'])
              .setTargeting('environment', ['desktop'])
              .setTargeting('section', ['content'])
              .setTargeting('content_category', ${formattedCategories});
            googletag.enableServices();
          });`}
      </Script>

      {/* Initialize Google Ads Display for Mobile Ads (Kept as is) */}
      {isMobileDevice ? (
        <Script id="gpt-display-mobile">
          {`
            googletag.cmd.push(function() {
        ...
        <Script id="gpt-display-desktop">
          {`
            googletag.cmd.push(function() {
              googletag.display('div-gpt-ad-1728872238496-0');
              googletag.display('div-gpt-ad-1728872258333-0');
              // googletag.display('div-gpt-ad-1728872497774-0'); // 970x250 - Replaced
              // googletag.display('div-gpt-ad-1728872758865-0'); // 300x250 - Replaced
            });
          `}
        </Script>
      )}
      ...
      <AppShell.Main>
        <Container size={1700} sx={{ display: 'flex', justifyContent: 'center' }}>
          <Grid>
            <Grid.Col span={isMobileDevice ? 12 : 9}>
              <div style={{ width: '100%' }}>
                {post && (
                  <div>
                    {post.featuredImage && (
                    ...
                    {/* Replace Large Middle Ad (970x250) with Temporary Ad */}
                    {hasMiddleAd && (
                      <>
                        {/* Original Ad Slot (Commented Out) */}
                        {/*
            ...
            {!isMobileDevice && (
              <Grid.Col span={3}>
                <div style={styles.banner}>
                  <SocialMediaLinks />
                  {/* 300x250 Ad - Replaced with Temporary Ad */}
                  {hasRightAd && (
                    <>
                      {/* Original Ad Slot (Commented Out) */}
                      {/*
