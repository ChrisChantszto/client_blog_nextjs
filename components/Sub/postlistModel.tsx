...
import { Text, Title, Divider, Image, Stack, Button, Grid, Container, Badge, Group, Center, Pagination, Box } from "@mantine/core";
...
interface PostlistModel {
  apiUrl: string;
  postsPerPage?: number;
  initialSortOrder?: 'newest' | 'mostHit';
  categories?: string[];
  pageName: string;
  showSocialLinks?: boolean;
}

export default function PostlistModel({
  apiUrl,
  postsPerPage = 4,
  initialSortOrder = 'newest',
  categories = ['全部', '玩樂', '飲食', '購物', '旅遊', '銀行', '會員'],
  pageName = '全部文章/大灣區旅遊',
  ...
  }, [currentPage, sortOrder, apiUrl]);

  const fetchPosts = async (page: number) => {
    try {
      const response = await axios.get(
        `${apiUrl}&page=${page}&number=${postsPerPage}&order=${sortOrder === 'newest' ? 'DESC' : 'ASC'}`
      );
      ...
      setTotalPages(Math.ceil(response.data.found / postsPerPage));
    } catch (error) {
  ...
  return (
    <Container fluid style={styles.container} >
      <Group justify="space-between">
        <Title fw={800} order={2}>{pageName}</Title>
        <Group gap="xs">
          <Text>排序:</Text>
          <Text
...
<br />
<Grid>
  <Grid.Col span={!isMobile && showSocialLinks ? 10 : 12}>
    <Stack
      align="stretch"
      justify="flex-start"
